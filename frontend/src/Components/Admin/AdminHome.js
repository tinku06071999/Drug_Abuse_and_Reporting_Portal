...
export default function AdminHome() {
  ...
  const pendingEmployees = employees.length - verifiedEmployees;
  const adminCount = employees.filter(e => e.roles?.includes("ADMIN")).length;

  // Student Support
  const uniqueStudents = new Set(responses.map(r => r.rollNumber)).size;
  ...
  const trend = useMemo(() => {
    ...
    return reportsPerDay.at(-1).count >
      reportsPerDay.at(-2).count
      ? "Increasing"
      : "Decreasing";
  }, [reportsPerDay]);
  ...
  return (
    <div className="text-slate-200 space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-6 gap-6">
        <StatCard icon={<BsFillArchiveFill />} label="Total Reports" value={totalReports} color="cyan" />
        <StatCard icon={<BsFillGrid3X3GapFill />} label="Resolved" value={resolvedReports} color="green" />
        <StatCard icon={<BsPeopleFill />} label="Pending" value={pendingReports} color="red" />
        <StatCard icon={<BsSpeedometer2 />} label="Resolution %" value={`${resolutionRate}%`} color="amber" />
        <StatCard icon={<BsPersonCheck />} label="Employees Verified" value={verifiedEmployees} color="green" />
        <StatCard icon={<BsEnvelopeOpen />} label="Support Messages" value={responses.length} color="cyan" />
      </div>

      {/* INSIGHT CARDS */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <InsightCard
          title="Employee Onboarding Health"
          value={`${verifiedEmployees} / ${employees.length} Verified`}
          status={pendingEmployees < 5 ? "Healthy" : "Attention"}
        />

        <InsightCard
          title="Student Engagement"
          value={`${uniqueStudents} Students`}
          helper={`Avg ${avgMsgPerStudent} messages/student`}
        />
